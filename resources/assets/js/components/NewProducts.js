...
{
    let previewNewSliderInited = false,
        sync1New, sync2New, navSpeedThumbs;

    function initPreviewNewSlider () {
        ...
        if ($('#new-products').length > 0) {
            ...
            if (!previewNewSliderInited === true) {
                sync1New = $("#new-products .sync1");
                sync2New = $("#new-products .sync2");
            }
            ...
            if (!previewNewSliderInited) {
                previewNewSliderInited = true;
            }
        }
        ...
        sync2New.on("click", ".owl-item", function (e) {
            e.preventDefault();
            let number = $(this).index();
            sync1New.trigger("to.owl.carousel", [number, 1000, true]);
            return false;
        });

        function center(num) {

            let sync2visible = sync2New.find('.owl-item.active').map(function () {
                return $(this).index();
            });

            if ($.inArray(num, sync2visible) === -1) {
                if (num > sync2visible[sync2visible.length - 1]) {
                    sync2New.trigger("to.owl.carousel", [num - sync2visible.length + 2, navSpeedThumbs, true]);
                } else {
                    sync2New.trigger("to.owl.carousel", Math.max(0, num - 1));
                }
            } else if (num === sync2visible[sync2visible.length - 1]) {
                sync2New.trigger("to.owl.carousel", [sync2visible[1], navSpeedThumbs, true]);
            } else if (num === sync2visible[0]) {
                sync2New.trigger("to.owl.carousel", [Math.max(0, num - 1), navSpeedThumbs, true]);
            }
        }
    }

    function destroyPreviewNewSlider () {
        if (previewNewSliderInited === true) {
            sync1New.trigger('destroy.owl.carousel');
            sync2New.trigger('destroy.owl.carousel');

            sync1New.find('.owl-stage-outer').children().unwrap();
            sync1New.removeClass("owl-center owl-loaded owl-text-select-on");

            sync2New.find('.owl-stage-outer').children().unwrap();
            sync2New.removeClass("owl-center owl-loaded owl-text-select-on");
        }
    }

    function checkPreviewNewSliderStock ()
    {
        GLOBAL_DATA.newProductPreview.product.product_sizes.forEach(function (item) {
            if (item.size_id == GLOBAL_DATA.newProductPreview.currentSizeId) {
                if (item.stocks[0].stock > 0) {
                    GLOBAL_DATA.newProductPreview.inStock = true;
                } else {
                    GLOBAL_DATA.newProductPreview.inStock = false;
                }
            }
        });
    }
    ...
    if (GLOBAL_DATA.newProducts && GLOBAL_DATA.newProducts.length > 0)
    {
        ...
        new Vue({
            ...
            methods: {
                ...
                toInteger: function (count) {
                    ...
                    GLOBAL_DATA.newProductPreview.product.product_sizes.forEach(function (item) {
                        if (item.product_id == searchObj.productId && item.size_id == searchObj.sizeId) {
                            ...
                            {
                                GLOBAL_DATA.newProductPreview.count = item.stocks[0].stock > 0 ? item.stocks[0].stock : 1;
                                showPopup(CART_MAX);
                            }
                        }
                    });
                ...
                addToCart: function (productId, sizeId, count) {
                    ...
                    if (GLOBAL_DATA.newProductPreview.inStock) {
                ...
                increment: function () {
                    ...
                    GLOBAL_DATA.newProductPreview.product.product_sizes.forEach(function (item) {
                        if (item.product_id == searchObj.productId && item.size_id == searchObj.sizeId) {
                            if (GLOBAL_DATA.newProductPreview.count < item.stocks[0].stock) {
                    ...
                    if (GLOBAL_DATA.newProductPreview.count > 9999) {
                        GLOBAL_DATA.newProductPreview.count = 9999;
                    }
