...
@if($model->countCategoryProducts >= $model->categoryProductsLimit)
    <div class="block-inline pagination-wrap text-center">
        <ul class="pagination-1">
            ...
            @if($isPrev)
                <li class="prv">
                    <a href="{{ url_category_filters_per_page($model->currentCategory->slug, $model->filtersParam, $model->page - 1, $model->language) }}"
                       class="disabled">
            ...
            @foreach($pages as $page)
                <li>
                    ...
                    @else
                        ...
                        @else
                            <a href="{{ url_category_filters_per_page($model->currentCategory->slug, $model->filtersParam, $page, $model->language) }}">
                                {{ $page }}
                            </a>
                        @endif
                    @endif
