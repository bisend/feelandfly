@extends('layout')

@section('content')
    <article class="light-bg-1">
        <!-- Slider Start -->
        <section class="slider-section">
            <div id="main-slider" class="main-slider nav-2">
                <div class="item">
                    <div class="carousel-inner white-mask">
                        <img src="/img/template/slider/slide-1.jpg" alt="...">
                        <div class="theme-container container">
                            <div class="caption-text tbl-wrp row">
                                <div class="text-middle">
                                    <div class="tbl-cell">
                                        <div class="col-xs-4 col-sm-4 hover-slider-cart-first">
                                            <div class="hover-cart">
                                                <a class="show-cart pulse"></a>
                                                <div class="prod-wrap-cont hover-slider-cart">
                                                    <div class="product_item prod-wrap">
                                                        <div class="product_img">
                                                            <div class="prod-img">
                                                                <a class="img-hover" href="#">
                                                                    <img alt="product"
                                                                         src="/img/template/product/category-1/1.jpg">
                                                                </a>
                                                                <div class="prod-tag-1 font-2">
                                                                        <span>
                                                                            Sale
                                                                        </span>
                                                                </div>
                                                                <a class="caption-link meta-icon"
                                                                   data-toggle="modal"
                                                                   href="#prod-preview">
                                                                    <span class="fa fa-eye"></span>
                                                                </a>
                                                            </div>
                                                        </div>
                                                        <div class="product_info">
                                                            <h2 class="prod-title">
                                                                <a href="product-detail.htm">
                                                                    ЗИППЕР FEEL&FLY STAR GRAY
                                                                </a>
                                                            </h2>
                                                            <div class="block-inline">
                                                                <div class="prod-price font-2">
                                                                    <ins>700.00 грн</ins>
                                                                </div>
                                                                <div class="rating">
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star"></span>
                                                                    <span class="star"></span>
                                                                </div>
                                                            </div>
                                                            <div class="block-inline">
                                                                <ul class="prod-meta">
                                                                    <li>
                                                                        <a class="theme-btn btn-black" href="#">
                                                                            Подробнее
                                                                        </a>
                                                                    </li>
                                                                    <li>
                                                                        <a class="fa fa-heart meta-icon" href="#"></a>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-xs-4 col-sm-4 hover-slider-cart-first">
                                            <div class="hover-cart">
                                                <a class="show-cart pulse"></a>
                                                <div class="prod-wrap-cont hover-slider-cart">
                                                    <div class="product_item prod-wrap">
                                                        <div class="product_img">
                                                            <div class="prod-img">
                                                                <a class="img-hover" href="#">
                                                                    <img alt="product"
                                                                         src="/img/template/product/category-1/1.jpg">
                                                                </a>
                                                                <div class="prod-tag-1 font-2">
                                                                    <span>Sale</span>
                                                                </div>
                                                                <a class="caption-link meta-icon"
                                                                   data-toggle="modal"
                                                                   href="#prod-preview">
                                                                    <span class="fa fa-eye"></span>
                                                                </a>
                                                            </div>
                                                        </div>
                                                        <div class="product_info">
                                                            <h2 class="prod-title">
                                                                <a href="product-detail.htm">
                                                                    ЗИППЕР FEEL&FLY STAR GRAY
                                                                </a>
                                                            </h2>
                                                            <div class="block-inline">
                                                                <div class="prod-price font-2">
                                                                    <ins>700.00 грн</ins>
                                                                </div>
                                                                <div class="rating">
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star"></span>
                                                                    <span class="star"></span>
                                                                </div>
                                                            </div>
                                                            <div class="block-inline">
                                                                <ul class="prod-meta">
                                                                    <li>
                                                                        <a class="theme-btn btn-black" href="#">
                                                                            Подробнее
                                                                        </a>
                                                                    </li>
                                                                    <li>
                                                                        <a class="fa fa-heart meta-icon" href="#"></a>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-xs-4 col-sm-4 hover-slider-cart-first">
                                            <div class="hover-cart">
                                                <a class="show-cart pulse"></a>
                                                <div class="prod-wrap-cont hover-slider-cart">
                                                    <div class="product_item prod-wrap">
                                                        <div class="product_img">
                                                            <div class="prod-img">
                                                                <a class="img-hover" href="#">
                                                                    <img alt="product"
                                                                         src="/img/template/product/category-1/1.jpg">
                                                                </a>
                                                                <div class="prod-tag-1 font-2">
                                                                    <span>Sale</span>
                                                                </div>
                                                                <a class="caption-link meta-icon"
                                                                   data-toggle="modal"
                                                                   href="#prod-preview">
                                                                    <span class="fa fa-eye"></span>
                                                                </a>
                                                            </div>
                                                        </div>
                                                        <div class="product_info">
                                                            <h2 class="prod-title">
                                                                <a href="product-detail.htm">
                                                                    ЗИППЕР FEEL&FLY STAR GRAY
                                                                </a>
                                                            </h2>
                                                            <div class="block-inline">
                                                                <div class="prod-price font-2">
                                                                    <ins>700.00 грн</ins>
                                                                </div>
                                                                <div class="rating">
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star"></span>
                                                                    <span class="star"></span>
                                                                </div>
                                                            </div>
                                                            <div class="block-inline">
                                                                <ul class="prod-meta">
                                                                    <li>
                                                                        <a class="theme-btn btn-black" href="#">
                                                                            Подробнее
                                                                        </a>
                                                                    </li>
                                                                    <li>
                                                                        <a class="fa fa-heart meta-icon" href="#"></a>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <div class="carousel-inner white-mask">
                        <img src="/img/template/slider/slide-3.jpg" alt="...">
                        <div class="theme-container container">
                            <div class="caption-text tbl-wrp">
                                <div class="text-middle">
                                    <div class="tbl-cell">
                                        <div class="col-xs-12 col-sm-12 text-center">
                                            <div class="hover-cart">
                                                <a class="show-cart pulse"></a>
                                                <div class="prod-wrap-cont hover-slider-cart">
                                                    <div class="product_item prod-wrap">
                                                        <div class="product_img">
                                                            <div class="prod-img">
                                                                <a class="img-hover" href="#">
                                                                    <img alt="product"
                                                                         src="/img/template/product/category-1/1.jpg">
                                                                </a>
                                                                <div class="prod-tag-1 font-2">
                                                                    <span>Sale</span>
                                                                </div>
                                                                <a class="caption-link meta-icon"
                                                                   data-toggle="modal"
                                                                   href="#prod-preview">
                                                                    <span class="fa fa-eye"></span>
                                                                </a>
                                                            </div>
                                                        </div>
                                                        <div class="product_info">
                                                            <h2 class="prod-title">
                                                                <a href="product-detail.htm">
                                                                    ЗИППЕР FEEL&FLY STAR GRAY
                                                                </a>
                                                            </h2>
                                                            <div class="block-inline">
                                                                <div class="prod-price font-2">
                                                                    <ins>700.00 грн</ins>
                                                                </div>
                                                                <div class="rating">
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star"></span>
                                                                    <span class="star"></span>
                                                                </div>
                                                            </div>
                                                            <div class="block-inline">
                                                                <ul class="prod-meta">
                                                                    <li>
                                                                        <a class="theme-btn btn-black" href="#">
                                                                            Подробнее
                                                                        </a>
                                                                    </li>
                                                                    <li>
                                                                        <a class="fa fa-heart meta-icon" href="#"></a>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <div class="carousel-inner white-mask">
                        <img src="/img/template/slider/slide-4.jpg" alt="...">
                        <div class="theme-container container">
                            <div class="caption-text tbl-wrp">
                                <div class="text-middle">
                                    <div class="tbl-cell">
                                        <div class="col-xs-12 col-sm-12 text-center">
                                            <div class="hover-cart">
                                                <a class="show-cart pulse"></a>
                                                <div class="prod-wrap-cont hover-slider-cart">
                                                    <div class="product_item prod-wrap">
                                                        <div class="product_img">
                                                            <div class="prod-img">
                                                                <a class="img-hover" href="#">
                                                                    <img alt="product"
                                                                         src="/img/template/product/category-1/1.jpg">
                                                                </a>
                                                                <div class="prod-tag-1 font-2">
                                                                    <span>Sale</span>
                                                                </div>
                                                                <a class="caption-link meta-icon"
                                                                   data-toggle="modal"
                                                                   href="#prod-preview">
                                                                    <span class="fa fa-eye"></span>
                                                                </a>
                                                            </div>
                                                        </div>
                                                        <div class="product_info">
                                                            <h2 class="prod-title">
                                                                <a href="product-detail.htm">
                                                                    ЗИППЕР FEEL&FLY STAR GRAY
                                                                </a>
                                                            </h2>
                                                            <div class="block-inline">
                                                                <div class="prod-price font-2">
                                                                    <ins>700.00 грн</ins>
                                                                </div>
                                                                <div class="rating">
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star active"></span>
                                                                    <span class="star"></span>
                                                                    <span class="star"></span>
                                                                </div>
                                                            </div>
                                                            <div class="block-inline">
                                                                <ul class="prod-meta">
                                                                    <li>
                                                                        <a class="theme-btn btn-black" href="#">
                                                                            Подробнее
                                                                        </a>
                                                                    </li>
                                                                    <li>
                                                                        <a class="fa fa-heart meta-icon" href="#"></a>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- / Slider Ends -->




        <!-- Latest Product  Start -->
        <section class="sec-space">
            <div class="container theme-container">
                <!-- Related Products Starts -->
                <div class="block-inline pt-15 similar_prod-section">
                    <h2 class="section-title">
                        Популярные Товары
                    </h2>
                    <div id="rel-prod-slider" class="rel-prod-slider nav-1 padding-own">
                        <div class="item similar_products">
                            <div class="prod-wrap pt-50">
                                <figure>
                                    <div class="prod-img">
                                        <a class="img-hover" href="#">
                                            <img alt="product" src="/img/template/product/category-1/2.jpg"></a>
                                        <div class="prod-tag-1 font-2">
                                            <span> -50% </span>
                                        </div>
                                        <a class="caption-link meta-icon"
                                           data-toggle="modal" href="#prod-preview">
                                            <span class="fa fa-eye"></span>
                                        </a>
                                    </div>
                                    <figcaption class="prod-content">
                                        <h2 class="prod-title">
                                            <a href="product-detail.htm">
                                                КУРТКА FEEL&FLY BUTTON BLACK
                                            </a>
                                        </h2>
                                        <div class="block-inline">
                                            <div class="rating">
                                                <span class="star active"></span>
                                                <span class="star active"></span>
                                                <span class="star active"></span>
                                                <span class="star"></span>
                                                <span class="star"></span>
                                            </div>
                                            <div class="prod-price font-2">
                                                <ins>480.00 грн</ins>
                                            </div>
                                        </div>
                                        <div class="block-inline">
                                            <ul class="prod-meta">
                                                <li>
                                                    <a class="theme-btn btn-black" data-toggle="modal" data-target=".big-cart">
                                                        подробнее
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="fa fa-heart meta-icon" href="#"></a>
                                                </li>
                                            </ul>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                        <div class="item similar_products">
                            <div class="prod-wrap pt-50">
                                <figure>
                                    <div class="prod-img">
                                        <a class="img-hover" href="#">
                                            <img alt="product" src="/img/template/product/category-1/2.jpg"></a>
                                        <div class="prod-tag-1 font-2">
                                            <span> -50% </span>
                                        </div>
                                        <a class="caption-link meta-icon"
                                           data-toggle="modal"
                                           href="#prod-preview">
                                            <span class="fa fa-eye"></span>
                                        </a>
                                    </div>
                                    <figcaption class="prod-content">
                                        <h2 class="prod-title">
                                            <a href="product-detail.htm">
                                                КУРТКА FEEL&FLY PRADO BLACK
                                            </a>
                                        </h2>
                                        <div class="block-inline">
                                            <div class="rating">
                                                <span class="star active"></span>
                                                <span class="star active"></span>
                                                <span class="star active"></span>
                                                <span class="star"></span>
                                                <span class="star"></span>
                                            </div>
                                            <div class="prod-price font-2">
                                                <ins>580.00 грн</ins> <del>760.00 грн</del>
                                            </div>
                                        </div>
                                        <div class="block-inline">
                                            <ul class="prod-meta">
                                                <li>
                                                    <a class="theme-btn btn-black" data-toggle="modal" data-target=".pop-up-messege">
                                                        подробнее
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="fa fa-heart meta-icon" href="#"></a>
                                                </li>
                                            </ul>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                        <div class="item similar_products">
                            <div class="prod-wrap pt-50">
                                <figure>
                                    <div class="prod-img">
                                        <a class="img-hover" href="#">
                                            <img alt="product" src="/img/template/product/category-1/3.jpg"></a>
                                        <a class="caption-link meta-icon"
                                           data-toggle="modal"
                                           href="#prod-preview">
                                            <span class="fa fa-eye"></span>
                                        </a>
                                    </div>
                                    <figcaption class="prod-content">
                                        <h2 class="prod-title">
                                            <a href="product-detail.htm">
                                                КУРТКА FEEL&FLY PRADO BLACK
                                            </a>
                                        </h2>
                                        <div class="block-inline">
                                            <div class="rating">
                                                <span class="star active"></span>
                                                <span class="star active"></span>
                                                <span class="star active"></span>
                                                <span class="star"></span>
                                                <span class="star"></span>
                                            </div>
                                            <div class="prod-price font-2">
                                                <ins>580.00 грн</ins> <del>760.00 грн</del>
                                            </div>
                                        </div>
                                        <div class="block-inline">
                                            <ul class="prod-meta">
                                                <li>
                                                    <a class="theme-btn btn-black" href="#">
                                                        подробнее
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="fa fa-heart meta-icon" href="#"></a>
                                                </li>
                                            </ul>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                        <div class="item similar_products">
                            <div class="prod-wrap pt-50">
                                <figure>
                                    <div class="prod-img">
                                        <a class="img-hover" href="#">
                                            <img alt="product" src="/img/template/product/category-1/6.jpg"></a>
                                        <div class="prod-tag-1 font-2">
                                            <span> -50% </span>
                                        </div>
                                        <a class="caption-link meta-icon"
                                           data-toggle="modal" href="#prod-preview">
                                            <span class="fa fa-eye"> </span>
                                        </a>
                                    </div>
                                    <figcaption class="prod-content">
                                        <h2 class="prod-title">
                                            <a href="product-detail.htm">
                                                БОМБЕР FEEL&FLY LITE BLACK
                                            </a>
                                        </h2>
                                        <div class="block-inline">
                                            <div class="rating">
                                                <span class="star active"></span>
                                                <span class="star active"></span>
                                                <span class="star active"></span>
                                                <span class="star"></span>
                                                <span class="star"></span>
                                            </div>
                                            <div class="prod-price font-2">
                                                <ins>580.00 грн</ins>
                                            </div>
                                        </div>
                                        <div class="block-inline">
                                            <ul class="prod-meta">
                                                <li>
                                                    <a class="theme-btn btn-black" href="#">
                                                        подробнее
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="fa fa-heart meta-icon" href="#"></a>
                                                </li>
                                            </ul>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- Related Products Ends -->


            </div>
        </section>
        <!-- / Latest Product Ends -->

        <!-- Special Promo Starts-->
        <section class="special-promo-sec black-mask">
            <a href="#" class="col-sm-3 col-xs-12 promo">
                <h2 class="section-title wht fsz-86">
                    01
                </h2>
                    <span class="sub-detail wht">
                        ВЕРХНЯЯ ОДЕЖДА
                    </span>
            </a>
            <a href="#" class="col-sm-3 col-xs-12 promo">
                <h2 class="section-title wht fsz-86">
                    02
                </h2>
                    <span class="sub-detail wht">
                        СВИТШОТЫ & ТОЛСТОВКИ
                    </span>
            </a>
            <a href="#" class="col-sm-3 col-xs-12 promo">
                <h2 class="section-title wht fsz-86">
                    03
                </h2>
                    <span class="sub-detail wht">
                        Штаны&Шорты
                    </span>
            </a>
            <a href="#" class="col-sm-3 col-xs-12 promo">
                <h2 class="section-title wht fsz-86">
                    04
                </h2>
                    <span class="sub-detail wht">
                        АКСЕССУАРЫ
                    </span>
            </a>
        </section>
        <!-- / Special Promo  Ends -->

        <!-- Product Slider Start -->
        <section class="prod-slider-sec pb-70">
            <div class="container theme-container">
                <div class="row">
                    <div class="col-md-6 col-sm-6 shares-products">
                        <h2 class="section-title">
                            Aкции
                        </h2>
                        <div id="prod-slider-1" class="prod-slider-1 nav-1">
                            <div class="item similar_products">
                                <div class="prod-wrap pt-50">
                                    <figure>
                                        <div class="prod-img">
                                            <a class="img-hover" href="#">
                                                <img alt="product" src="/img/template/product/category-1/6.jpg"></a>
                                            <div class="prod-tag-1 font-2">
                                                <span> -50% </span>
                                            </div>
                                            <a class="caption-link meta-icon"
                                               data-toggle="modal"
                                               href="#prod-preview">
                                                <span class="fa fa-eye"> </span>
                                            </a>
                                        </div>
                                        <figcaption class="prod-content">
                                            <h2 class="prod-title">
                                                <a href="product-detail.htm">
                                                    БОМБЕР FEEL&FLY LITE BLACK
                                                </a>
                                            </h2>
                                            <div class="block-inline">
                                                <div class="rating">
                                                    <span class="star active"></span>
                                                    <span class="star active"></span>
                                                    <span class="star active"></span>
                                                    <span class="star"></span>
                                                    <span class="star"></span>
                                                </div>
                                                <div class="prod-price font-2">
                                                    <ins>580.00 грн</ins>
                                                </div>
                                            </div>
                                            <div class="block-inline">
                                                <ul class="prod-meta">
                                                    <li>
                                                        <a class="theme-btn btn-black" href="#">
                                                            подробнее
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a class="fa fa-heart meta-icon" href="#"></a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>
                            <div class="item similar_products">
                                <div class="prod-wrap pt-50">
                                    <figure>
                                        <div class="prod-img">
                                            <a class="img-hover" href="#">
                                                <img alt="product" src="/img/template/product/category-1/6.jpg"></a>
                                            <div class="prod-tag-1 font-2">
                                                <span> -50% </span>
                                            </div>
                                            <a class="caption-link meta-icon"
                                               data-toggle="modal"
                                               href="#prod-preview">
                                                <span class="fa fa-eye"> </span>
                                            </a>
                                        </div>
                                        <figcaption class="prod-content">
                                            <h2 class="prod-title">
                                                <a href="product-detail.htm">
                                                    БОМБЕР FEEL&FLY LITE BLACK
                                                </a>
                                            </h2>
                                            <div class="block-inline">
                                                <div class="rating">
                                                    <span class="star active"></span>
                                                    <span class="star active"></span>
                                                    <span class="star active"></span>
                                                    <span class="star"></span>
                                                    <span class="star"></span>
                                                </div>
                                                <div class="prod-price font-2">
                                                    <ins>580.00 грн</ins>
                                                </div>
                                            </div>
                                            <div class="block-inline">
                                                <ul class="prod-meta">
                                                    <li>
                                                        <a class="theme-btn btn-black" href="#">
                                                            подробнее
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a class="fa fa-heart meta-icon" href="#"></a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-6 new-products">
                        <h2 class="section-title">
                            Новинки
                        </h2>
                        <div id="prod-slider-2" class="prod-slider-2 nav-1">
                            <div class="item similar_products">
                                <div class="prod-wrap pt-50">
                                    <figure>
                                        <div class="prod-img">
                                            <a class="img-hover" href="#">
                                                <img alt="product" src="/img/template/product/category-1/2.jpg"></a>
                                            <div class="prod-tag-1 font-2">
                                                <span> -50% </span>
                                            </div>
                                            <a class="caption-link meta-icon"
                                               data-toggle="modal"
                                               href="#prod-preview">
                                                <span class="fa fa-eye"> </span>
                                            </a>
                                        </div>
                                        <figcaption class="prod-content">
                                            <h2 class="prod-title">
                                                <a href="product-detail.htm"> КУРТКА FEEL&FLY BUTTON BLACK </a> </h2>
                                            <div class="block-inline">
                                                <div class="rating">
                                                    <span class="star active"></span>
                                                    <span class="star active"></span>
                                                    <span class="star active"></span>
                                                    <span class="star"></span>
                                                    <span class="star"></span>
                                                </div>
                                                <div class="prod-price font-2">
                                                    <ins>480.00 грн</ins>
                                                </div>
                                            </div>
                                            <div class="block-inline">
                                                <ul class="prod-meta">
                                                    <li> <a class="theme-btn btn-black" href="#"> подробнее </a> </li>
                                                    <li> <a class="fa fa-heart meta-icon" href="#"></a> </li>
                                                </ul>
                                            </div>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>
                            <div class="item similar_products">
                                <div class="prod-wrap pt-50">
                                    <figure>
                                        <div class="prod-img">
                                            <a class="img-hover" href="#">
                                                <img alt="product" src="/img/template/product/category-1/2.jpg"> </a>
                                            <div class="prod-tag-1 font-2">
                                                <span> -50% </span>
                                            </div>
                                            <a class="caption-link meta-icon"
                                               data-toggle="modal"
                                               href="#prod-preview">
                                                <span class="fa fa-eye"> </span>
                                            </a>
                                        </div>
                                        <figcaption class="prod-content">
                                            <h2 class="prod-title">
                                                <a href="product-detail.htm">
                                                    КУРТКА FEEL&FLY BUTTON BLACK
                                                </a>
                                            </h2>
                                            <div class="block-inline">
                                                <div class="rating">
                                                    <span class="star active"></span>
                                                    <span class="star active"></span>
                                                    <span class="star active"></span>
                                                    <span class="star"></span>
                                                    <span class="star"></span>
                                                </div>
                                                <div class="prod-price font-2">
                                                    <ins>480.00 грн</ins>
                                                </div>
                                            </div>
                                            <div class="block-inline">
                                                <ul class="prod-meta">
                                                    <li> <a class="theme-btn btn-black" href="#"> подробнее </a> </li>
                                                    <li> <a class="fa fa-heart meta-icon" href="#"></a> </li>
                                                </ul>
                                            </div>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- / Product Slider Ends -->

    </article>
@endsection